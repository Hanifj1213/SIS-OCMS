    ...
    public function updateStage(Request $request, Component $component)
    {
        ...
        $result = $transition->inTransaction(function () use ($request, $component, $transition) {
            ...
            $usesGsheetChecksheet = ($currentStage == 2
                && (
                    (bool) $locked->gsheet_url
                    || (
                        $locked->major_category === 'Engine'
                        && strtoupper(trim((string) $locked->egi)) === 'PC2000-8'
                    )
                ))
            ...
            if ($currentStage == 2 && !$locked->gsheet_measurement_url && !$locked->gsheet_subassy_measurement_url) {
                $request->validate([
                ...
                foreach ($request->parts as $partData) {
                    $locked->inspectionDetails()->create([
                ...
                app(FabricationRequestService::class)->createFromInspectionDetails(
                    $locked->fresh(['inspectionDetails', 'fabricationRequests']),
                    auth()->id()
                );
            }
